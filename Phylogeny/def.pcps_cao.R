pcps_cao <- function (comm, dist.spp, method = "bray", 
                      squareroot = TRUE, cailliez_trans = FALSE) 
{
  dis <- dist.spp
  ...
  if (cailliez_trans == TRUE) {
    P.dist <- cailliez(P.dist)
  }
  ...
  if (length(unique(ordi.P$eig < 0)) > 1) {
    warning("Warning: Negative eigenvalues are present in the decomposition result, but only positive eigenvalues were considered", 
            call. = FALSE)
  }
  ...
  Res <- list(call = match.call(), P = P, values = Values, 
              vectors = vectors, correlations = correlations)
  class(Res) <- "pcps"
  return(Res)
}